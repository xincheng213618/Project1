...
void CUDA_Init_ALL(int nH, int nW)
{
	...
	//创建cufft句柄
	cufftPlan2d(&cufftForwrdHandle, 4*nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftInverseHandle, 4*nH, nW, CUFFT_C2C);
}
//
//void CUDA_Free_ALL()
//{
...
//生成网格并计算平方根
void CUDA_grid_creat_now(cv::Mat* dst, int nH, int nW, float delta_x, float delta_y)
{
	float* device1_float;
...
void CUDA_circ_creat(cv::Mat src, cv::Mat &dst1, cv::Mat &dst2, float Max_frequency, float Filter)
{
	int nH = src.rows;
	int nW = src.cols;
	int Nt = nH * nW;
	...
	float* device1_float, * device2_float, * device3_float;
