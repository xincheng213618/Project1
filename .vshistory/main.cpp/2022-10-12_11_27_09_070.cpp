...
struct show
{
	cv::Mat Show_PC = cv::Mat::zeros(960+int(round(0.03f * 960) * 2), 1280 + int(round(0.03f * 1280) * 2), CV_32FC1);
};
...
cv::Mat Phase2PC(cv::Mat _phi, float Max_frequency, float Pixelsize, float Filter, int i) {//, float Gamma, float Gain, 
	...
	weight = Mat2Gray(weight, 0.25f, 0.9f, 2);
	weight.convertTo(weight, CV_8UC1, 1, 0);
	return weight;
}
...
int main()
{
	...
	for (int i = 0; i < 4; i++) {
		...
		std::thread([&]() {
			int p_in = *p++;
			M[i].Show_PC = Phase2PC(image[j], Max_frequency, Pixelsize, Filter, i);
			/*sprintf_s(str, sizeof(str), "E:\\实验室学习\\Task4-22.08.04\\CV\\封装\\old\\write\\%d.jpg", p_in);
