...
int main()
{
	...
	for (int i = 0; i < 4; i++) {
		int j = i % 3;
		std::thread([&]() {
			Show_PC[i] = Phase2PC(image[j], Max_frequency, Pixelsize, Filter, i);
			}).detach();
	}
	std::this_thread::sleep_for(std::chrono::milliseconds(15000));
	for (int i = 0; i < 4; i++)
	{
		sprintf_s(str, sizeof(str), "E:\\实验室学习\\Task4-22.08.04\\CV\\封装\\old\\write\\%d.jpg", *p++);
		cv::imwrite(str, Show_PC[i]);
	}
