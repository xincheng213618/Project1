...
static float(*device1_float)[4], (*device2_float)[4], (*device3_float)[4], (*device4_float)[4];
float(*Amp_real)[4], (*Phi_imag)[4];
cufftComplex(*U0_device)[4];
...
{
	Check(cudaMalloc((void**)&device1_float, 4 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&device2_float, 4 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&device3_float, 4 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&device4_float, 4 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&Amp_real, 4 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&Phi_imag, 4 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&U0_device, 4 * nH * nW * sizeof(cufftComplex)));
	////创建cufft句柄
...
void CUDA_grid_creat_now(cv::Mat* dst, int nH, int nW, float delta_x, float delta_y, int i)
{
	...
	grid_creat_kernel << <grid, block, 0, stream[i] >> > (device1_float[i], nH, nW, delta_x, delta_y, nH * nW);
	Check(cudaMemcpy(rho.data, (uchar*)device1_float[i], nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	*dst = rho;
...
void CUDA_circ_creat(cv::Mat src, cv::Mat& dst1, cv::Mat& dst2, float Max_frequency, float Filter, int i)
{
	...
	Check(cudaMemcpy(device3_float[i], src.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice));
	dim3 grid((nH * nW + 1024 - 1) / 1024);//(1280*960+1024-1)/1024
	dim3 block(1024);//1024
	filter_creat_kernel << <grid, block, 0, stream[i] >> > (device3_float[i], device1_float[i], device2_float[i], Max_frequency, Filter, Denoise_Radius, Nt);
	Check(cudaMemcpy(weight.data, (uchar*)device1_float[i], nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	Check(cudaMemcpy(filter2.data, (uchar*)device2_float[i], nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	dst1 = weight;
	dst2 = filter2;
	cudaStreamSynchronize(0);
}
...
void CUDA_filter_creat_now(cv::Mat src1, cv::Mat src2, cv::Mat& dst1, cv::Mat& dst2, bool flag, int i)
{
	...
	Check(cudaMemcpy(device1_float[i], src1.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice));
	Check(cudaMemcpy(device2_float[i], src2.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice));

	dim3 grid((nH * nW + 1024 - 1) / 1024);//(1280*960+1024-1)/1024
	dim3 block(1024);//1024
	Phase_delay << <grid, block, 0, stream[i] >> > (device1_float[i], flag, Nt);
	Mat2complex_kernel << <grid, block, 0, stream[i] >> > (device1_float[i], U0_device[i], 1.0f, Nt);
	Complex2mat_kernel << <grid, block, 0, stream[i] >> > (U0_device[i], Amp_real[i], Phi_imag[i], false, Nt);

	CUDA_mul_kernel << <grid, block, 0, stream[i] >> > (Amp_real[i], device2_float[i], Amp_real[i], Nt);
	CUDA_mul_kernel << <grid, block, 0, stream[i] >> > (Phi_imag[i], device2_float[i], Phi_imag[i], Nt);
	
	//fftshift real
	fftshift_step1_kernel << <grid, block, 0, stream[i] >> > (Amp_real[i], device1_float[i], device2_float[i], device3_float[i], device4_float[i], nH, nW, Nt);
	fftshift_step2_kernel << <grid, block, 0, stream[i] >> > (device1_float[i], device2_float[i], device3_float[i], device4_float[i], Amp_real[i], nH, nW, Nt);
	Check(cudaMemcpy(real_out.data, (uchar*)Amp_real[i], nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	dst1 = real_out;
	//fftshift imag
	fftshift_step1_kernel << <grid, block, 0, stream[i] >> > (Phi_imag[i], device1_float[i], device2_float[i], device3_float[i], device4_float[i], nH, nW, Nt);
	fftshift_step2_kernel << <grid, block, 0, stream[i] >> > (device1_float[i], device2_float[i], device3_float[i], device4_float[i], Phi_imag[i], nH, nW, Nt);
	Check(cudaMemcpy(imag_out.data, (uchar*)Phi_imag[i], nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	dst2 = imag_out;
	cudaStreamSynchronize(0);
}
