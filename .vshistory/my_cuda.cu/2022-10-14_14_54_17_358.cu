...
void CUDA_ALL_calculate(cv::Mat Phi, cv::Mat real_filter, cv::Mat imag_filter, cv::Mat* Ipc, int nH_extend, int nW_extend, int i)
{
	...
	Check(cudaMemcpy(Phi_imag, Phi.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice, stream[i]));
	Check(cudaMemcpy(Amp_real, real_filter.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice, stream[i]));
	...
	Check(cudaMemcpy(Phi_imag, imag_filter.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice, stream[i]));
