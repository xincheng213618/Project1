...
static float* device1_float_out = NULL, * device2_float_out = NULL, * device3_float_out = NULL, * device4_float_out = NULL;
float* Amp_real_out = NULL, * Phi_imag_out = NULL;
cufftComplex* U0_device_out = NULL;
...
{
	Check(cudaMalloc((void**)&device1_float_out, 2 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&device2_float_out, 2 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&device3_float_out, 2 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&device4_float_out, 2 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&Amp_real_out, 2 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&Phi_imag_out, 2 * nH * nW * sizeof(float)));
	Check(cudaMalloc((void**)&U0_device_out, 2 * nH * nW * sizeof(cufftComplex)));
	////创建cufft句柄
...
{
	printf("abc = %d", i);
	float* device1_float = (device1_float_out + i * nH * nW * sizeof(float));
...
void CUDA_circ_creat(cv::Mat src, cv::Mat& dst1, cv::Mat& dst2, float Max_frequency, float Filter, int i)
{
	...
	float* device1_float = device1_float_out + i * nH * nW * sizeof(float);
	float* device2_float = device2_float_out + i * nH * nW * sizeof(float);
	float* device3_float = device3_float_out + i * nH * nW * sizeof(float);
...
void CUDA_filter_creat_now(cv::Mat src1, cv::Mat src2, cv::Mat& dst1, cv::Mat& dst2, bool flag, int i)
{
	...
	float* device2_float = device2_float_out + i * nH * nW * sizeof(float);
	float* device3_float = device3_float_out + i * nH * nW * sizeof(float);
	float* device4_float = device4_float_out + i * nH * nW * sizeof(float);
	float* Amp_real = Amp_real_out + i * nH * nW * sizeof(float);
	float* Phi_imag = Phi_imag_out + i * nH * nW * sizeof(float);
	cufftComplex* U0_device = U0_device_out + i * nH * nW * sizeof(cufftComplex);
