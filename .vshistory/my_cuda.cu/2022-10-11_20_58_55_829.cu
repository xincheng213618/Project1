...
void CUDA_filter_creat_now(cv::Mat src1, cv::Mat src2, cv::Mat & dst1, cv::Mat & dst2, bool flag)
{
	int nH = src1.rows;
	int nW = src1.cols;
	int Nt = nH * nW;

	float* device1_float, * device2_float, * device3_float, * device4_float;
	float* Amp_real, * Phi_imag;
