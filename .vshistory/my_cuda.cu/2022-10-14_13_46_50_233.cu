...
void CUDA_Init_ALL(int nH, int nW)
{
	...
	for (int i = 0; i < 4; i++) {
		cufftPlan2d(&cufft_Handle[i], nH, nW, CUFFT_C2C);
	}
...
void CUDA_ALL_calculate(cv::Mat Phi, cv::Mat real_filter, cv::Mat imag_filter, cv::Mat* Ipc, int nH_extend, int nW_extend, int i)
{
	...
	cufftSetStream(cufft_Handle[i], stream[i]);
	...
	//执行fft正变换
	cufftExecC2C(cufft_Handle[i], U0_device, U0_device, CUFFT_FORWARD);
	...
	cufftExecC2C(cufft_Handle[i], U0_device, U0_device, CUFFT_INVERSE);
