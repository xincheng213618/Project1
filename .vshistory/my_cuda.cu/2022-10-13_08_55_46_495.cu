...
//尝试 实现fftshift
__global__ void fftshift_step1_kernel(float* src, float* dst, int nH, int nW, int N)
{
	float* tmp1 = new float[nH * nW], float* tmp2 = new float[nH * nW];
	float* tmp3 = new float[nH * nW], float* tmp4 = new float[nH * nW];
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	if (idx < N) {
		if ((idx % nW < nW / 2) && (idx / nW < nH / 2))
		{
			tmp1[idx / nW * (nW / 2) + idx % nW] = src[idx];
		}
		else if ((idx % nW >= nW / 2) && (idx / nW < nH / 2))
		{
			tmp2[idx / nW * (nW / 2) + (idx - nW / 2) % nW] = src[idx];
		}
		else if ((idx % nW < nW / 2) && (idx / nW >= nH / 2))
		{
			tmp3[idx / nW * (nW / 2) - nH / 2 * nW / 2 + idx % nW] = src[idx];
		}
		else
		{
			tmp4[idx / nW * (nW / 2) - nH / 2 * nW / 2 + (idx - nW / 2) % nW] = src[idx];
		}
		__syncthreads();
		if ((idx % nW < nW / 2) && (idx / nW < nH / 2))
		{
			dst[idx] = tmp4[idx / nW * (nW / 2) + idx % nW];
		}
		else if ((idx % nW >= nW / 2) && (idx / nW < nH / 2))
		{
			dst[idx] = tmp3[idx / nW * (nW / 2) + (idx - nW / 2) % nW];
		}
		else if ((idx % nW < nW / 2) && (idx / nW >= nH / 2))
		{
			dst[idx] = tmp2[idx / nW * (nW / 2) - nH / 2 * nW / 2 + idx % nW];
		}
		else
		{
			dst[idx] = tmp1[idx / nW * (nW / 2) - nH / 2 * nW / 2 + (idx - nW / 2) % nW];
		}
	}
}
...
void CUDA_filter_creat_now(cv::Mat src1, cv::Mat src2, cv::Mat& dst1, cv::Mat& dst2, bool flag, int i)
{
	...
	//fftshift real
	fftshift_step1_kernel << <grid, block >> > (Amp_real, Amp_real, nH, nW, Nt);
	...
	dst1 = real_out;
	//fftshift imag
	fftshift_step1_kernel << <grid, block >> > (Phi_imag, Amp_real, nH, nW, Nt);
