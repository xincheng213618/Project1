...
static float(*device1_float_out)[4], (*device2_float)[4], (*device3_float)[4], (*device4_float)[4];
...
void CUDA_grid_creat_now(cv::Mat* dst, int nH, int nW, float delta_x, float delta_y, int i)
{
	...
	grid_creat_kernel << <grid, block >> > (device1_float[i], nH, nW, delta_x, delta_y, nH * nW);
...
void CUDA_circ_creat(cv::Mat src, cv::Mat& dst1, cv::Mat& dst2, float Max_frequency, float Filter, int i)
{
	...
	dim3 grid((nH * nW + 1024 - 1) / 1024);//(1280*960+1024-1)/1024
	dim3 block(1024);//1024
	filter_creat_kernel << <grid, block >> > (device3_float[i], device1_float[i], device2_float[i], Max_frequency, Filter, Denoise_Radius, Nt);
...
void CUDA_filter_creat_now(cv::Mat src1, cv::Mat src2, cv::Mat& dst1, cv::Mat& dst2, bool flag, int i)
{
	...
	float* device1_float; device1_float = device1_float_out[i];
	float*device2_float = NULL, * device3_float = NULL, * device4_float = NULL;
	...
	dim3 grid((nH * nW + 1024 - 1) / 1024);//(1280*960+1024-1)/1024
	dim3 block(1024);//1024
	Phase_delay << <grid, block >> > (device1_float[i], flag, Nt);
	Mat2complex_kernel << <grid, block >> > (device1_float[i], U0_device[i], 1.0f, Nt);
	Complex2mat_kernel << <grid, block >> > (U0_device[i], Amp_real[i], Phi_imag[i], false, Nt);

	CUDA_mul_kernel << <grid, block >> > (Amp_real[i], device2_float[i], Amp_real[i], Nt);
	CUDA_mul_kernel << <grid, block >> > (Phi_imag[i], device2_float[i], Phi_imag[i], Nt);
	
	//fftshift real
	fftshift_step1_kernel << <grid, block >> > (Amp_real[i], device1_float[i], device2_float[i], device3_float[i], device4_float[i], nH, nW, Nt);
	fftshift_step2_kernel << <grid, block >> > (device1_float[i], device2_float[i], device3_float[i], device4_float[i], Amp_real[i], nH, nW, Nt);
	Check(cudaMemcpy(real_out.data, (uchar*)Amp_real[i], nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	dst1 = real_out;
	//fftshift imag
	fftshift_step1_kernel << <grid, block >> > (Phi_imag[i], device1_float[i], device2_float[i], device3_float[i], device4_float[i], nH, nW, Nt);
	fftshift_step2_kernel << <grid, block >> > (device1_float[i], device2_float[i], device3_float[i], device4_float[i], Phi_imag[i], nH, nW, Nt);
