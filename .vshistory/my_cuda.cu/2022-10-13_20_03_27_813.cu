...
void CUDA_Init_ALL(int nH, int nW)
{
	...
	for (int i = 0; i < 4; i++)
	{
		cufftPlan2d(&cufftForwrdHandle[i], nH, nW, CUFFT_C2C);
		cufftPlan2d(&cufftInverseHandle[i], nH, nW, CUFFT_C2C);
	}
}
//
void CUDA_Free_ALL()
{
	for (int i = 0; i < 4; i++)
	{
		cufftDestroy(cufftForwrdHandle[i]); //销毁句柄，对应cufftPlan2d
		cufftDestroy(cufftInverseHandle[i]);//销毁句柄，对应cufftPlan2d
	}
