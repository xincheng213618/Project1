...
void CUDA_Init_ALL(int nH, int nW)
{
	for (int i = 0; i < 10; i++)
	{
		Check(cudaMalloc((void**)&CUDA[i]->device1_float, nH * nW * sizeof(float)));
		Check(cudaMalloc((void**)&CUDA[i], nH * nW * sizeof(CUDA_V)));
	}
...
void CUDA_grid_creat_now(cv::Mat* dst, int nH, int nW, float delta_x, float delta_y, int i)
{
	...
	grid_creat_kernel << <grid, block >> > (CUDA[i]->device1_float, nH, nW, delta_x, delta_y, nH * nW);
	Check(cudaMemcpy(rho.data, (uchar*)CUDA[i]->device1_float, nH * nW * sizeof(float), cudaMemcpyDeviceToHost));
	*dst = rho;
