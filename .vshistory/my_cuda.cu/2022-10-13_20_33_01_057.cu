...
cufftHandle cufftForwrdHandle[4], cufftInverseHandle[4];
...
void CUDA_Init_ALL(int nH, int nW)
{
	...
	//创建cufft句柄
	cufftPlan2d(&cufftForwrdHandle[0], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftForwrdHandle[1], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftForwrdHandle[2], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftForwrdHandle[3], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftInverseHandle[0], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftInverseHandle[1], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftInverseHandle[2], nH, nW, CUFFT_C2C);
	cufftPlan2d(&cufftInverseHandle[3], nH, nW, CUFFT_C2C);
...
void CUDA_Free_ALL()
{
	...
	cufftDestroy(cufftForwrdHandle[0]); //销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftForwrdHandle[1]); //销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftForwrdHandle[2]); //销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftForwrdHandle[3]); //销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftInverseHandle[0]);//销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftInverseHandle[1]);//销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftInverseHandle[2]);//销毁句柄，对应cufftPlan2d
	cufftDestroy(cufftInverseHandle[3]);//销毁句柄，对应cufftPlan2d
}
...
void CUDA_ALL_calculate(cv::Mat Phi, cv::Mat real_filter, cv::Mat imag_filter, cv::Mat* Ipc, int nH_extend, int nW_extend, int i)
{
	...
	//执行fft正变换
	cufftExecC2C(cufftForwrdHandle[i], U0_device, U0_device, CUFFT_FORWARD);
	...
	cufftExecC2C(cufftInverseHandle[i], U0_device, U0_device, CUFFT_INVERSE);
