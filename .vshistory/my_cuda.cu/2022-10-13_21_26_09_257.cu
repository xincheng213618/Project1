...
void CUDA_ALL_calculate(cv::Mat Phi, cv::Mat real_filter, cv::Mat imag_filter, cv::Mat* Ipc, int nH_extend, int nW_extend, int i)
{
	...
	Check(cudaMemcpyAsync(Phi_imag, Phi.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice));
	Check(cudaMemcpyAsync(Amp_real, real_filter.data, nH * nW * sizeof(float), cudaMemcpyHostToDevice));
