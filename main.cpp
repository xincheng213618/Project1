...
cv::Mat Phase2PC(cv::Mat _phi, float Max_frequency, float Pixelsize, float Filter) {//, float Gamma, float Gain, 
	...
	stop = clock();
	std::cout << "ALL_calculate: " << (double)(stop - start) / CLOCKS_PER_SEC << endl;
...
int main()
{
	...
	for (int i = 0; i < 50; i++)
	{
		...
		cout << "Onetime() detail:" << (double)(end - start) / CLOCKS_PER_SEC << endl;
	}
	cout << "alltime:" << (double)(end - start1) / CLOCKS_PER_SEC << endl;
